...
The `*.svg` files in this directory are **placeholders** generated by
[`../../scripts/gen-placeholders.mjs`](../../scripts/gen-placeholders.mjs).
They show a stylised, brand-coloured stand-in so the site looks intentional
during development. **Replace them with real AI-generated artwork** before
deploying.

The two files under `screenshots/` are **real dashboard captures**, copied
verbatim from the main repo's `docs/screenshots/`. Re-run the main repo's
`scripts/screenshots.mjs` whenever the dashboard UI changes.

## Workflow for replacing a placeholder

1. Use the prompt below in Midjourney / DALL-E 3 / Stable Diffusion / Ideogram.
2. Pick the result you like; export at the listed resolution (2× retina).
3. Save as **WebP** (smaller, transparent-friendly). PNG is also fine.
4. Drop the new file at `public/images/<name>.webp`.
5. In `site/src/pages/`, find the `<FeatureCard imageSrc="<name>.svg">`
   reference and change `.svg` → `.webp` (a one-shot sed works:
   `find site/src/pages -name "*.astro" -exec sed -i '' 's/<name>\.svg/<name>.webp/g' {} +`).

## Prompt catalogue

All prompts target the same **dark-indigo aesthetic** — `#0A0A0A`
background, Indigo-400 (`#818CF8`) accents, soft glows, isometric or
flat-vector style. The goal is "premium dev-tool marketing illustration"
along the lines of Vercel / Linear / Stripe.
